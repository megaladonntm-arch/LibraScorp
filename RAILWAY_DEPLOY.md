...
3. Optional but recommended: create a **Volume** and mount it to `/data` for persistent SQLite.
...
- Without a Volume, `DB_PATH=bot.sqlite3` works but data can be lost on redeploy/restart.
- With a Volume, set `DB_PATH=/data/bot.sqlite3` for persistent storage.
...
- If using Volume: mounted to `/data`
- If using Volume: DB file exists at `/data/bot.sqlite3`
